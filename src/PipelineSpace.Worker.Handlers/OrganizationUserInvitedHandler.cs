...
{
    public class OrganizationUserInvitedHandler : IEventHandler<OrganizationUserInvitedEvent>
    {
        ...
        public OrganizationUserInvitedHandler(IOptions<ApplicationOptions> applicationOptions,
        ...
        public async Task Handle(OrganizationUserInvitedEvent @event)
        {
            ...
            //else
            //{
            //    template = $"Please join to PipelineSpace to accept the invitation by clicking this link: <a href='{HtmlEncoder.Default.Encode($"{url}/Account/Join/{@event.OrganizationUserInvitationId}")}'>link</a>";
            //}

            //await _emailWorkerService.SendEmailAsync(@event.UserEmail, "PipelineSpace Organization Invitation", template);
        }
    }
}
